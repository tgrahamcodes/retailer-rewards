        ...
        <tbody>
          {invoice.products.map((p, i) => (
            <tr key={i} className="border-b border-gray-100">
              ...
              <td className="py-1.5 text-right text-gray-700">${p.price.toFixed(2)}</td>
            </tr>
          ))}
        </tbody>
        <tfoot>
          <tr className="font-bold text-gray-800">
            ...
            <td className="pt-3 text-right text-green-600">${invoice.total.toFixed(2)}</td>
          </tr>
        </tfoot>
      </table>
...
// This function is to display the receipts.
function CustomerReceipts() {
  const { id } = useParams();
  const [summary, setSummary] = useState(null);
  const [error, setError] = useState(null);

  // API call to get the customer, by customer ids' invoices.
  useEffect(() => {
    fetch(`/api/customer/${id}/invoices`)
      .then((res) => res.json())
      .then((data) => {
        if (!data || data.months.length === 0) setError(`No receipts found for customer #${id}`);
      ...
      <div className="mb-8">
        ...
        <div className="bg-white rounded-2xl shadow-md p-6 flex gap-12">
          <div>
            ...
            <p className="text-2xl font-bold text-gray-800">${summary.totalSpent.toFixed(2)}</p>
          </div>
        <div className="ml-auto">
          <p className="text-xs text-gray-400 uppercase mb-1">Total Rewards</p>
          <p className="text-2xl font-bold text-green-600">{summary.totalRewards} pts</p>
        </div>
        </div>
      </div>
        ...
        <div key={i} className="mb-10">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-6 text-sm text-gray-500">
              <span>Spent: <span className="font-semibold text-gray-800">${month.totalSpent.toFixed(2)}</span></span>
              <span>Points: <span className="font-semibold text-green-600">{month.pointsEarned} pts</span></span>
            </div>
          </div>
